...
export default function FinanzasTaller() {
  ...
  // ══ QUIZ MODE ══════════════════════════════
  if (screen === "quizmode") return <QuizMode xp={xp} setXp={setXp} rewardEnabled={!completedModules.quiz} onBack={() => setScreen("home")} onGoGame={() => { resetGame(); setScreen("game"); }} onGoLearn={() => setScreen("learn")} onGoHome={() => setScreen("home")} onComplete={() => setCompletedModules(m => ({ ...m, quiz: true }))} />;
    ...
    return (
      <div style={appSt}>
        ...
        <div style={{ padding: "38px 18px" }}>
          ...
          <button onClick={() => setScreen("home")} style={{ width: "100%", padding: 15, borderRadius: 14, background: C.bg, border: `1px solid ${C.border}`, color: C.textMid, fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: "inherit", marginBottom: 10 }}>
            Ir al inicio 🏠
          </button>
